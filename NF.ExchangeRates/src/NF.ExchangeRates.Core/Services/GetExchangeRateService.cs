...
namespace NF.ExchangeRates.Core.Services
{
    ...
    {
        private readonly Func<ApiProviders, IExchangeRateRetriever> _retriever;
        ...
        public GetExchangeRateService(Func<ApiProviders, IExchangeRateRetriever> retriever, ILogger<GetExchangeRateService> logger,
        ...
        private async Task<ExchangeRate> GetFromDbOrProvider(string from, string to, ApiProviders provider, CancellationToken cancellationToken = default)
        {
            _logger.LogInformation("Attempting to get rates from cache");
            var key = $"{provider}_{from}_{to}";
            try
            {
                ...
                var rate = await _reader.Read(provider, from, to, cancellationToken);
                if (rate != null)
                {
            ...
            var providerRate = (await _retriever(provider).GetExchangeRate(provider, from, to, cancellationToken));
